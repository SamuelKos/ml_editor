...
proc make_editor { {file ""} {display_window 1} {highlight 0} } {
	...
	# text-widget carousel BEGIN ###########################################
	#################################
	# Set string/name/windowpath for main text-widget,
	# Save it to variable: t
	set t $f1.text
	# Save it also to variable editor($editor_no,text)
...
##            -wrap none -font $font -tabs "$editor(tabwidth)" -cursor $cur -background #e7e7e7
	...
	####################################
	# Type of variable t changes yet again in this proc.
	# Create proc, name it to literal: $t
	# --> now when:
	# 0: calling text-widget saved in variable t, for example: $t see 1.0
	# 1: instead proc $t (right below) gets called
	# This proc is quite short, only filtering insert and delete actions
	####################################
	
	
	# Currently, proxying below is not in use to gain some speed
...
##
##		######################################################
##		# Here: evaluate command in real text-widget
##		# (meaning: doing normal text-widget stuff)
...
##		#################### text-widget carousel END ##############
	...
	##########################################
	# Update screen/display status after every key/button release
...
##	}
	...
	# Double click on text brace: Select the braces
...
##	syntax_highlight $editor_no $startline $endline
##
##	set t2 [clock seconds]
##	set t1t2 [expr {$t2-$t1}]
##	puts "$editor_no: syntax highlight took $t1t2 seconds"
##	validate_procedures $editor_no
##
...
##	# store line number where insert/delete starts
##	if {[string equal $command insert] || [string equal $command delete]} {
##		set line1 [lindex [split [$t index insert] .] 0]
##
##		if {!$editor($editor_no,writable)} {
##			bell
##			return ""
...
##	# Here: evaluate command in real text-widget
##	# (meaning: doing normal text-widget stuff)
##	# set result [eval uplevel [list $t $command $args]]
##	######################################################
##	
##	if {[string equal $command insert] || [string equal $command delete]} {
##		# insert/delete? syntax highlight the newly inserted text & checkall procedures
##		set line2 [lindex [split [$t index insert] .] 0]
##		#syntax_highlight $editor_no $line1 $line2
##		validate_procedures $editor_no
##		set editor($editor_no,status) MODIFIED
##		$t see insert
...
proc check_indent_depth {w} {
	set contents [$w get insert {insert +20 lines}]
	#set contents $w

	# Avainsanat, joita etsitään rivin alusta (tai sisennyksen jälkeen)
	...
	for {set i 0} {$i < $num_lines} {incr i} {
		...
		# Tarkistetaan löytyykö jokin avainsanoista ja päättyykö rivi kaksoispisteeseen
		set found_word 0
		foreach word $words {
		...
		if {!$found_word || [string index $trimmed_line end] ne "\{"} {
			continue
		}

		# Etsitään seuraava ei-tyhjä rivi (offset)
		set next_line ""
		for {set offset [expr {$i + 1}]} {$offset < $num_lines} {incr offset} {
			set temp [lindex $lines $offset]
			if {[string trim $temp] ne ""} {
				set next_line $temp
				break
			}
		}

		if {$next_line eq ""} continue

		# Lasketaan nykyisen rivin sisennys (indent_0)
		set indent_0 [expr {[string length $line] - [string length [string trimleft $line " \t"]]}]

		# Lasketaan seuraavan rivin sisennys (indent_1) ja tarkistetaan välilyönnit
		set indent_1 0
		set flag_space 0

		# Käydään läpi seuraavan rivin alkumerkit
		...
		puts "line $i, indent diff: $indent_diff"
		return ""

##        # Testit: Jos sisennys ei kasva tai on tabulaattoreita (yli 1 merkki kerrallaan)
##        if {$indent_diff <= 0 || (!$flag_space && $indent_diff > 1)} {
##            continue
##        }

##        # Jos käytössä ei ole välilyöntejä, kaikki on kunnossa (False, 0)
##        if {!$flag_space} {
##            return [list 0 0]
##        }

##        # Jos löytyi välilyöntejä, verrataan niitä olion omaan asetukseen (ind_depth)
##        # Oletetaan, että $ind_depth on luokan muuttuja
##        if {$indent_diff != $ind_depth} {
##            return [list 1 $indent_diff]
##        } else {
##            return [list 0 0]
##        }
...
##set l [split "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_.$:" ""]
##set l [lsort $l]
##set alnums $l


set editor(version) "1.12"
